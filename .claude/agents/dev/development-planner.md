...
- 수락 기준과 완료 조건 정의
- **🚨 구현 완료 후에는 예외 없이 반드시 테스트를 수행한다 (테스트 없는 구현은 미완료로 간주)**
- **🚨 모든 테스트는 Playwright MCP를 사용하여 수행한다**
- **API 연동·비즈니스 로직 Task는 정상 케이스뿐 아니라 실패/예외/엣지 케이스까지 빠짐없이 테스트 시나리오를 작성한다**
- 테스트 통과를 확인한 뒤에만 해당 단계를 완료로 표시한다
...
- **API 연동·비즈니스 로직 작업은 "## 테스트 체크리스트" 섹션을 반드시 포함하며, 다음을 Playwright MCP 기준으로 구체적으로 작성:**
  - **정상 케이스**: 핵심 기능이 의도대로 동작하는 시나리오
  - **실패/예외 케이스**: API 오류, 네트워크 실패, 권한 없음, 데이터 누락 등
  - **엣지 케이스**: 경계값, 빈 데이터, 대용량, 잘못된 입력 등
  - 각 시나리오에 대한 검증 대상(화면 요소, 응답, 상태)을 명시
...
- 작업 파일의 명세서를 따름
- 기능과 기능성 구현
- **🚨 구현이 끝나면 반드시 테스트를 수행한다 — 테스트는 선택이 아닌 필수 단계다**
- **🚨 모든 테스트는 Playwright MCP를 사용한다 (browser_navigate, browser_snapshot, browser_click, browser_network_requests 등 활용)**
- API 연동·비즈니스 로직은 "## 테스트 체크리스트"의 정상/실패/엣지 케이스를 모두 실행하여 검증
- 테스트 중 발견된 버그는 수정 후 재테스트하여 통과를 확인
- 테스트 통과를 확인한 뒤에만 해당 단계를 완료로 표시하고 다음 단계로 진행
...
API 연동, 데이터 파싱, 비즈니스 로직 등 **외부 의존성이나 로직이 포함된 Task**는 아래 원칙을 반드시 따른다.

#### **테스트 원칙**

1. **구현 후 즉시 테스트**: 어떤 구현이든 완료되면 곧바로 테스트를 수행한다. 테스트하지 않은 구현은 "미완료"다.
2. **Playwright MCP 사용**: 모든 테스트는 Playwright MCP 도구로 실제 브라우저에서 실행한다.
3. **체크리스트 기반 검증**: 작업 파일의 "## 테스트 체크리스트"에 정의된 모든 시나리오를 실제로 실행하여 통과를 확인한다.
4. **버그 발견 시 수정-재테스트 반복**: 통과할 때까지 수정과 재테스트를 반복한다.

#### **Playwright MCP 활용 패턴**

- `browser_navigate`: 대상 페이지(예: `/q/[pageId]`) 접속
- `browser_snapshot`: 렌더링된 DOM 구조 및 데이터 표시 확인
- `browser_click` / `browser_type`: 다운로드 버튼 클릭 등 사용자 인터랙션 검증
- `browser_network_requests`: API 호출(Notion API, `/api/pdf/[pageId]`) 요청·응답 검증
- `browser_console_messages`: 런타임 에러 및 경고 확인
- `browser_take_screenshot`: 시각적 결과물(견적서 레이아웃 등) 확인

#### **테스트 체크리스트 작성 양식 (작업 파일 내)**

```markdown
## 테스트 체크리스트

### 정상 케이스
- [ ] 유효한 pageId로 접속 시 견적서가 정상 렌더링되는가 (browser_navigate → browser_snapshot)
- [ ] 금액 합계(소계/세액/총액)가 올바르게 계산·표시되는가
- [ ] PDF 다운로드 버튼 클릭 시 파일이 정상 생성되는가 (browser_network_requests)

### 실패/예외 케이스
- [ ] 존재하지 않는 pageId 접속 시 오류 안내 페이지로 이동하는가
- [ ] 비공개/권한 없는 페이지 접속 시 적절한 메시지가 표시되는가
- [ ] Notion API 오류 시 graceful하게 처리되는가

### 엣지 케이스
- [ ] 견적 항목이 0개이거나 매우 많을 때 레이아웃이 깨지지 않는가
- [ ] 선택적 필드(주소, 메모 등)가 비어 있을 때 정상 표시되는가
```
